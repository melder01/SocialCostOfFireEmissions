...
Params.horizon = ceil(Params.numyears/Params.timestep); % number of periods over which will optimize policy; default: 500/Params.timestep
...
Params.damexp = 2; %The exponent for the damage function (default: 2)
...
switch Params.damagemodel 
    ...
    case 'other'
        %damage coefficient loaded in GUI
    otherwise
...
%Constrain abatement rate if not optimizing
if Params.opt == 0
    Params.bound_abate = fixedAbate * ones(Params.horizon,1); % upper bound on abatement rate
end

if Params.opt == 2
    targetPeriods = ceil((targDate - 2015) / Params.timestep) + 1;
    Params.bound_abate = ones(Params.horizon,1); % upper bound on abatement rate
    Params.bound_abate([1:targetPeriods],1) = linspace(0.1695, 1, targetPeriods);
end
...
Fun.Ynet = @(tfp,L,K,T) Fun.Ygross(tfp,L,K).*(1-Params.damcoeff*T.^Params.damexp);
...
Fun.dYnet_dT = @(tfp,L,K,T) -((Params.damexp)*Params.damcoeff*T.^(Params.damexp-1)).*Fun.Ygross(tfp,L,K);
